...
function createTarget (payload, cb) {
  redisClient.hlen(repoName, (err, length) => {
    ...
    payload.accept.hour = payload.accept.hour.map(Number)
...
function update (id, payload, cb) {
  redisClient.hget(repoName, id, (err, data) => {
    ...
    let { geoState, hour, ..._clientData } = payload

    if (hour) hour = hour.map(Number)

    const updatedData = {
      ..._DBdata,
      ..._clientData,
      accept: {
        hour: hour || accept.hour,
        geoState: geoState || accept.geoState
      }
    }
