...
function acceptValidator (payload = {}) {
  let isValid = true
  let msg = ''
  const keys = payload.accept ? Object.keys(payload.accept) : []

  if (keys.length === 0) {
    msg = '"accept" value, must be provided'
    isValid = false
    return { isValid, msg }
  }

  for (let i = 0; i < keys.length; i++) {
    if (!payload.accept[keys[i]] || !Array.isArray(payload.accept[keys[i]]) || payload.accept[keys[i]].length === 0) {
      msg = `"accept" field value, must be an array with values, got ${keys[i]}: ${payload.accept[keys[i]]}`
      isValid = false
