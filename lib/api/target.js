...
const { sendErrorResponse, sendSuccessResponse, inputFilter, getHour, getAvailableTargets, sortTargetsByValue } = require('../utils')
...
function createTarget (req, res, opts) {
  jsonBody(req, res, (err, data) => {
    ...
    const errors = validate(data, ['url', 'value', 'maximumAcceptPerDay', 'geoState', 'hour'])
...
function requestPost (req, res, opts) {
  jsonBody(req, res, (err, data) => {
    ...
    const errors = validate(data, ['timestamp'])
    if (errors.length !== 0) return sendErrorResponse(req, res, errors, 406)

    const hr = getHour(timestamp)

    targetRepo.getAll((err, data) => {
      if (err) return sendErrorResponse(req, res)

      const targets = getAvailableTargets(data, hr, geoState)
      const [highestTarget] = sortTargetsByValue(targets)
